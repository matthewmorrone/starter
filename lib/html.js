...
Html.unescape = function(str) { //from underscore.string
	if (str) {
		str = toString(str)
			.replace(/&amp;/g , '&')
			.replace(/&lt;/g  , '<')
...
Html.prototype.get_html_translation_table = function(table, quote_style) {
	var entities = {},
		hash_map = {},
		decimal;
...
Entity.escape = function(str) {
	str.replace(/&(.+?);/g, function(str, ent) String.fromCharCode( ent[0]!='#' ? entityToCode[ent] : ent[1]=='x' ? parseInt(ent.substr(2),16): parseInt(ent.substr(1)) ) );
}
Entity.unescape = function(str) {
	str.replace(/[^\x20-\x7E]/g, function(str) charToEntity[str] ? '&'+charToEntity[str]+';' : str );
}
